...
export default class CssCompiler{
    ...
    static #listClasses = {
        ...
        bs: { prop: 'border-style', unit:this.#units.nonNumeric.NULL, priority:17, isSetUnit:false },
        bsh: { prop: ['border-left-style', 'border-right-style'], unit:this.#units.nonNumeric.NULL, priority:18, isSetUnit:false },
        bsv: { prop: ['border-top-style', 'border-bottom-style'], unit:this.#units.nonNumeric.NULL, priority:19, isSetUnit:false },
        bst: { prop: 'border-top-style', unit:this.#units.nonNumeric.NULL, priority:20, isSetUnit:false },
        bsr: { prop: 'border-right-style', unit:this.#units.nonNumeric.NULL, priority:21, isSetUnit:false },
        bsb: { prop: 'border-bottom-style', unit:this.#units.nonNumeric.NULL, priority:22, isSetUnit:false },
        bsl: { prop: 'border-left-style', unit:this.#units.nonNumeric.NULL, priority:23, isSetUnit:false },
        bw: { prop: 'border-width', unit: this.#units.numeric.PX, priority:24, isSetUnit:true},
        bwh: { prop: ['border-left-width', 'border-right-width'], unit:this.#units.numeric.PX, priority:25, isSetUnit:true},
        bwv: { prop: ['border-top-width', 'border-bottom-width'], unit:this.#units.numeric.PX, priority:26, isSetUnit:true},
        bwt: { prop: 'border-top-width', unit:this.#units.numeric.PX, priority:27, isSetUnit:true },
        bwr: { prop: 'border-right-width', unit:this.#units.numeric.PX, priority:28, isSetUnit:true },
        bwb: { prop: 'border-bottom-width', unit:this.#units.numeric.PX, priority:29, isSetUnit:true },
        bwl: { prop: 'border-left-width', unit:this.#units.numeric.PX, priority:30, isSetUnit:true },
        bc: { prop: 'border-color', unit: this.#units.nonNumeric.COLOR, priority:31, isSetUnit:false },
        bch: { prop: ['border-left-color', 'border-right-color'], unit: this.#units.nonNumeric.COLOR, priority:32, isSetUnit:false },
        bcv: { prop: ['border-top-color', 'border-bottom-color'], unit: this.#units.nonNumeric.COLOR, priority:33, isSetUnit:false },
        bct: { prop: 'border-top-color', unit: this.#units.nonNumeric.COLOR, priority:34, isSetUnit:false },
        bcr: { prop: 'border-right-color', unit: this.#units.nonNumeric.COLOR, priority:35, isSetUnit:false },
        bcb: { prop: 'border-bottom-color', unit: this.#units.nonNumeric.COLOR, priority:36, isSetUnit:false },
        bcl: { prop: 'border-left-color', unit: this.#units.nonNumeric.COLOR, priority:37, isSetUnit:false },
        ...
        w: { prop: 'width', unit:this.#units.numeric.PR, priority:46, isSetUnit:true},
        ...
        z: { prop: 'z-index', unit: this.#units.numeric.NONE, priority:78, isSetUnit:false, precision:0} ,
    ...
    static #regClasses = [
        ...
        /\s{1}(md|mdl|mdp)?-?(ai|as|ajc)(-i)?pseudo-(start(:flex-start){0}|center|end(:flex-end){0}|stretch|inherit|initial)\s{1}/g,
        ...
        /\s{1}(md|mdl|mdp)?-?(flex|basis|grow|shrink|h|minh|maxh|w|minw|maxw|bwt|bwr|bwb|bwl|bwh|bwv|bw|brad|bradtl|bradbl|bradtr|bradbr|op|fsz|lh)(-i)?pseudo-(\d+(_\d+)?|auto|inherit|initial|unset|none)units.numeric\s{1}/g,
    ...
    static #regReplacedValues = /([\w\-]+)\(:([\w\-]+)\)/g
    ...
    static #getFiles(pathStr, ext){ 
        try{
            ...
            pathes = pathes.map(p=>path.isAbsolute(p) ? p : path.join(pathStr, p).replace(/\\/g, '/'))
        ...
        try{
            ...
            if (allClasses.length)
                for (let reg of this.#regClasses) {
                    ...
                    arrReplacedValues = [...arrReplacedValues, ...Array.from(reg.toString().matchAll(this.#regReplacedValues)).map(m => [m[1], m[2]])]
                }
                ...
                for (let match of matches) {
                    ...
                    if (this.#listClasses?.[cls]) {
                        ...
                        if (p.unit == this.#units.nonNumeric.COLOR)
                            value = /^[a-f0-9]{3,8}$/.test(match[6]) ? `#${match[6]}` 
                                    : (Object.keys(this.#config.SET_NAMES).includes(match[6]) 
                                        ? this.#config.SET_NAMES[match[6]] 
                                        : match[6])
                        else if (Object.values(this.#units.nonNumeric).includes(p.unit))
                            value = keysRValues.includes(match[6]) 
                                        ? replacedValues[match[6]] 
                                        : (Object.keys(this.#config.SET_NAMES).includes(match[6]) 
                                            ? this.#config.SET_NAMES[match[6]] 
                                            : match[6])
                        else {
